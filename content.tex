...
Need $\gsssem !=! \sssem$ for static inputs!
...
\forall \langle \grad{\pi_{pre}}, \grad{\pi_{post}} \rangle \in \gsssem^{\grad{s}}.~ \evalgphiGen{\grad{\pi_{pre}}}{\grad{\phi_{pre}}} \implies \evalgphiGen{\grad{\pi_{post}}}{\grad{\phi_{post}}} \\
	 ...
	 & \forall \pi_{pre}, \pi_{post} \in \setProgramState.~ \sssem^s(\pi_{pre}) = \pi_{post} \implies \evalphiGen{\pi_{pre}}{\phi_{pre}} \implies \evalphiGen{\pi_{post}}{\phi_{post}}                                                                                                 \\
	 & \forall \pi_{pre}, \pi_{post} \in \setProgramState.~ \gsssem^s(\pi_{pre}) = \pi_{post} \implies \evalphiGen{\pi_{pre}}{\phi_{pre}} \implies \evalphiGen{\pi_{post}}{\phi_{post}}
\end{flalign*} 
NOT A SOUND LIFTING!
...
\input{text/SEC-abstracting-dynamic-semantics}
    ...
    \begin{mathpar}
        ...
        {
            \grad{\pi} \in \wsp(\grad{s_1}) \\ 
        }
        {
            \exists n \in \setNat,\, \grad{s_2} \in \setGStmt.~ \gsssem^n(\grad{\pi}) \in \wsp(\grad{s_2})
        }
    \end{mathpar}
    \begin{mathpar}
