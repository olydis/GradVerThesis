...
Note that this runtime check dynamically verify a method contract that has \ttt{this.balance >= amount} as precondition.
...
The method contract does not state how \ttt{this.balanced} is changed during a call to \ttt{withdrawCoins}, meaning that it cannot prove the precondition for the second call (we assume that the static verifier exclusively relies on the contract while reasoning about a method call).
An additional postcondition \ttt{ensures this.balance = old(this.balance) - amount} would solve that problem, resulting in overall success of the verification.
...
This is realized by giving \ttt{withdrawCoins} an unknown postcondition $\qm$, which will be treated as any plausible formula (in this example, one that proves that the balance is still high enough).
