...
When first introducing \tset{GDPreservation} in section \ref{ssec:the-deterministic-approach}, we also gave an example illustrating that whether it holds or not can depend on the small-step semantics of \gvl.
The example demonstrates that it can be a problem if $\sssem$ is defined even for program states for which Hoare logic cannot derive further progress.
The solution is to artificially reduce the definedness of $\sssem$ without changing the semantics of \svl, as described in the following sections.
