...
\begin{definition}[Completeness for Statement]~\\
    A semantics is \textbf{complete for statement $s$} if every execution of $s$ can be supported with a matching Hoare logic derivation:
    \begin{flalign*}
    & \forall \pi_1, \pi_2 \in \setProgramState.~ \sstepConsume{s}{\pi_1}{\pi_2} \implies \exists \phi_1, \phi_2 \in \setFormula.~ \thoare{}{\phi_1}{s}{\phi_2} \wedge \evalphiGen{\pi_1}{\phi_1}
    \end{flalign*}
    Note that from soundness of the Hoare logic it immediately follows that $\evalphiGen{\pi_2}{\phi_2}$ holds.
\end{definition}
\begin{definition}[Completeness]~\\
    \label{def:completeness}
    A semantics is \textbf{complete} if it is complete for every statement.
    \begin{flalign*}
