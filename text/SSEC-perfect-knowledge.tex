...
\end{flalign*}
Intuitively, $\sssem'$ is the same as $\sssem$, but is artificially made undefined/exceptional whenever there exists no Hoare rule that would be able to deduce static information about the execution.

\begin{lemma}[Restricted Domain of Small-Step Semantics]
    Replacing the small-step \sssem semantics of \svl with \sssem' as defined above would have no observable effects.
\end{lemma}



 ~$\sssem : \setProgramState \rightharpoonup \setProgramState$, but stuck for all .
Let ~$\gsssem : \setGProgramState \rightarrow \setGProgramState$~ be defined as
\
$\gsssem(\grad{\pi}) = \pi_{EX}$ if stuck for all concretizations.
