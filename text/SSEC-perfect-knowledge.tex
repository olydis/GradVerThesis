...
\end{flalign*}
Intuitively, $\sssem'$ is the same as $\sssem$, but is artificially made exceptional whenever there exists no Hoare rule that would be able to deduce static information about the execution.
...
Let ~$\gsssem : \setGProgramState \rightarrow \setGProgramState$ be a total gradual lifting of $\sssem'$ that is optimal in its exceptional behavior:
\begin{displaymath}
\forall \grad{\pi}.~ (\forall \pi \in \gamma(\grad{\pi}).~ \sssem'(\pi) \in \setProgramStateEx) \implies \gsssem(\grad{\pi}) \in \setProgramStateEx
\end{displaymath}

\begin{theorem}
    If the gradual small-step semantics $\gsssem$ of \gvl are defined as above, then \gvl satisfies \tset{GDPreservation}.
\end{theorem}
