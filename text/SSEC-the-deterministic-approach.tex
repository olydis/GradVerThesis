...
\begin{description}
    ...
    \end{mathpar}
    Note however, that this rule is not yet guaranteed to be satisfied, since this also depends on the gradual dynamic semantics.
    ...
    On the other hand, adding a runtime assertion check that throws an exception on failure would restore preservation.
\end{description}