...
\item[Static Semantics]~\\
    ...
    \begin{definition}[Weakest Static Precondition]~\\
        Let $\wsp : \setStmts \rightarrow \PP(\setProgramState)$ be defined as
        \begin{displaymath}
        \wsp(\overline{s}) = \{~ \pi \in \setProgramState ~|~ \exists \phi_1, \phi_2 \in \setFormula.~ \thoare{~}{\phi_1}{\overline{s}}{\phi_2} ~~\wedge~~ \evalphiGen{\pi}{\phi_1} ~\}
        \end{displaymath}
    \end{definition}
    Intuitively, the $\wsp(\overline{s})$ is a predicate on program states, indicating whether we could deduce anything about the state after executing $\overline{s}$, using only our Hoare rules.
    We require that $\wsp$ is computable at least for individual statements and that the resulting predicate is computable as well.
...
\item[Dynamic Semantics]~\\
    ...
    To simplify reasoning about program states, we assume $$\forall \pi_X \in \setProgramStateEx, \phi \in \setFormula.~ \evalphiGen{\pi_X}{\phi}$$
